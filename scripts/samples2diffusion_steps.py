...
def image_grid(array, ncols=4):
    ...
    img_grid = (array.reshape(nrows, ncols, height, width, channels).swapaxes(
        1, 2).reshape(height * nrows, width * ncols, channels))
...
if __name__ == '__main__':
    ...
    out_dir = (Path(args.out_dir) if args.out_dir is not None else Path(
        args.samples_dir).parent) / out_dir
    out_dir.mkdir(exist_ok=True)
    random_str = uuid.uuid4()

    filenames = Path(args.samples_dir).glob('all_timestep_sample_*.npy')
    filenames = list(filenames) + list(
        Path(args.samples_dir).glob('video_*.npy'))
    print(filenames)
    for filename in sorted(filenames)[:args.do_n]:
        ...
        (
            ...
            total_frames,
            observed_frames,
        ) = args.samples_dir.split('/')[-2].split('_')[:5]

        plt.title(
            f'Sampling method: {sampling_method} Sliding window: {max_frames} # Generated frames: {step_size} Video length: {total_frames} Ground truth: {observed_frames}'
        )
