...
def submit(remaining_steps, args):
    ...
    if args.slurm_min_array > 0:
        remaining_steps = [
            x for x in remaining_steps if x >= args.slurm_min_array
        ]
    ...
    array_arg = (
        '--array ' + ','.join(map(str, remaining_steps)) +
        ('' if args.slurm_max_array is None else f'%{args.slurm_max_array}'))
    ...
    cmd = ' '.join(
        [SUBMISSION_CMD, submission_args, '--', 'python', script_args])
...
def get_mse_random(
    ...
    for batch, _ in dataloader:
        ...
        t_seq = np.array([[
            np.random.RandomState(inference_step * 1000 + cnt + i).randint(
        ...
        metrics = run_bpd_evaluation(
            ...
            batch=batch,
            clip_denoised=True,
            obs_indices=obs_incides[:len(batch)],
            lat_indices=lat_indices[:len(batch)],
            t_seq=t_seq,
        )
        metrics = {
            k: v / t_seq.shape[1] * diffusion.num_timesteps
...
def get_mse_linspace(
    ...
    computes model's MSE in predicting the latent frames given the observed
    frames for different videos taken from the dataloader and different DDPM
    timesteps, on a grid with size specified by num_timesteps.
    ...
    lat_indices = [latent_frame_indices for _ in range(dataloader.batch_size)]
    mse_all = []
    t_seq_all = (diffusion.num_timesteps - 1 - np.linspace(
        0, diffusion.num_timesteps, num_timesteps, endpoint=False, dtype=int))
    video_cnt = 0
    for batch, _ in dataloader:
        batch = batch.to(device)

        t_seq = t_seq_all.take(range(video_cnt, video_cnt + len(batch)),
                               mode='wrap')
...
def update_schedule_on_disk(schedule_path, schedule, force=True):
    with test_util.Protect(schedule_path):
        ...
        saved_schedule = torch.load(
            schedule_path) if schedule_path.exists() else {}
        for k, v in schedule.items():
            if force:
                assert (k not in saved_schedule
                        ), f'Found {k} in the saved schedule! {schedule_path}'
            saved_schedule[k] = v
...
def main(args, model, diffusion, dataset, schedule_path, verbose=True):
    ...
    inference_schedule = (
        {}
    )  # A dictionary from "inference step" to the list of optimal observed frame indices.
    schedule_path = Path(schedule_path)
    partial_schedule_path = schedule_path.parent / ('.' + schedule_path.stem +
                                                    '_partial.pt')
    ...
    if partial_schedule_path.exists():
        ...
        partial_schedule = {}
    for cnt, (_, latent_frame_indices) in enumerate(
            tqdm(frame_indices_iterator, leave=False, desc='Inference step')):
        if task_id is not None and cnt != task_id:
            print(
                f'Skipping inference step {cnt}; not for this instance of array job.'
            )
            continue
        if cnt in saved_schedule:
            print(f'Skipping inference step {cnt}; already done.')
            continue
        n_to_condition_on = frame_indices_iterator._max_frames - len(
            latent_frame_indices)
        obs_frame_indices = (
            set()
            if cnt not in partial_schedule else set(partial_schedule[cnt])
        )  # Pre-fill the obs_frame_indices with the partial schedule
        ...
        while len(obs_frame_indices) < min(
                len(frame_indices_iterator._done_frames), n_to_condition_on):
            ...
            if 'linspace-t' in args.optimality:
                ...
                indices = np.random.RandomState(cnt * 1000 +
                                                len(obs_frame_indices)).choice(
                                                    len(dataset),
                                                    args.subset_size,
                                                    replace=False)
                assert (
                    len(indices) % args.num_timesteps == 0
                ), f'Subset size should ({len(indices)}) be divisible by the number of timesteps ({args.num_timesteps}).'
            elif 'random-t' in args.optimality:
                raise NotImplementedError(
                    'We decided to not use random-t anymore due to its high variance.'
                )
            else:
            ...
            metrics = (
                {}
            )  # Will be a dictionary of the form {frame_idx: {diffusion_t: [list of metric values]}}
            for candidate_idx in tqdm(frame_indices_iterator._done_frames,
                                      desc='Candidate index'):
                if (candidate_idx in latent_frame_indices
                        or candidate_idx in obs_frame_indices):
                ...
                metrics[candidate_idx] = get_metric_fn(
                    latent_frame_indices=latent_frame_indices,
                    obs_frame_indices=list(obs_frame_indices) +
                    [candidate_idx],
                    model=model,
                    diffusion=diffusion,
                    dataloader=dataloader,
                ...
                if verbose:
                    cur_metrics_array = np.array(
                        list(metrics[candidate_idx].values()))
                    cur_metrics_array = cur_metrics_array.mean(axis=0)
                    print(
                        f'(Step #{cnt}) Candidate frame {candidate_idx}: ({cur_metrics_array.mean(axis=0)}, {cur_metrics_array.std(axis=0)}) --- Latent: {latent_frame_indices} --- Observed: {list(obs_frame_indices)}'
                    )
            # Transfrom metrics form the dictionary format to a list of piars of (candidate_idx, candidate_avg_metric)
            metrics = [(candidate_idx,
                        np.array(list(candidate_metrics.values())).mean())
            ...
            print(
                f'(Step #{cnt}) Best frame {best_idx}, metric = {best_metric}')
            # Update the partial schedule
            update_schedule_on_disk(
                schedule_path=partial_schedule_path,
                schedule={cnt: list(obs_frame_indices)},
                force=False,
            )
        ...
        update_schedule_on_disk(schedule_path=schedule_path,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=10,
        help=
        'Number of timesteps to use for estimating the ELBO. Only used in mse-linsapce.',
    )
    parser.add_argument(
        '--step',
        type=int,
        default=None,
        help=
        'Which step of inference to produce optimal observations for. Used for parallel sampling on multiple machines.',
    )
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=
        'If given, figures out which steps of the optimal schedule are remaining to be optimized, then submits an array job doing them.',
    )
    parser.add_argument(
        '--slurm_cluster',
        type=str,
        choices=['plai', 'plai_towers', 'cc', 'submit-ml'],
        default=None,
    )
    parser.add_argument(
        '--slurm_max_array',
        ...
        default=None,
        help=
        'If given, will use it as a limit on the number of concurrently running array jobs.',
    )
    parser.add_argument(
        '--slurm_min_array',
        type=int,
        default=0,
        help=
        "Specifies the minimum index allowed to use for SLURM's --array argument.",
    )
    ...
    # Required for get_eval_run_identifier to work correctly.
    args.optimal = True
    ...
    if args.submit:
        ...
        saved_schedule = {} if not schedule_path.exists() else torch.load(
            schedule_path)
        frame_indices_iterator = inference_util.inference_strategies[
            args.inference_mode](
                video_length=args.T,
        ...
        remaining_steps = [
            step for step in range(num_steps) if step not in saved_schedule
        ]
