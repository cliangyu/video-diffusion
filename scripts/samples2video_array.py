...
if __name__ == '__main__':
    ...
    out_dir = (Path(args.out_dir) if args.out_dir is not None else
               Path(args.samples_dir).parent / 'video_arrays')
    out_dir.mkdir(exist_ok=True)
    random_str = uuid.uuid4()
    dirname = args.samples_dir.split('/')[-3]
    out_path = (
        out_dir /
        f'{args.dataset}_{dirname}_{args.do_n}_{args.n_seeds}_{args.obs_length}.{args.format}'
    )
    ...
    for video_i in range(args.do_n):  # [5, 17, 18, 10, 15]
        ...
        while done < args.n_seeds:
            filename = Path(
                args.samples_dir) / f'sample_{video_i:04d}-{seed}.npy'
            print(filename)
            try:
