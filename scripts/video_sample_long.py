...
def infer_step(model, diffusion, video, args):
    ...
    video = (video / 255.) * (drange[1] - drange[0]) - 1 # Normalize to [-1, 1]
    video = video[:args.obs_length]
    video = torch.tensor(video)
    assert len(video) == args.obs_length, f"Expected {args.obs_length} frames, but got {len(video)}"
    T, C, H, W = video.shape
    ...
    samples = (samples - drange[0]) / (drange[1] - drange[0])  * 255 # samples with pixel values in [0, 255]
...
if __name__ == "__main__":
    ...
    (model, diffusion), model_args = test_util.load_checkpoint(
        ...
        device=args.device, use_ddim=args.use_ddim,
    ...
    if not any(args.out.iterdir()):
        ...
        assert args.starting_video is None, "--starting_video argument is not allowed when the output directory is not empty."
        args.starting_video = sorted(list(args.out.glob("video_*.npy")), key=lambda x: int(x.stem.split("_")[1]))[-1]
        args.starting_video = Path(args.starting_video)
        video_index_offset = int(args.starting_video.stem.split("_")[1]) + 1
        assert config_path.exists(), f"Model config file {config_path} does not exist."
        with open(config_path, "r") as f:
            loaded_args = json.load(f)
        assert vars(model_args) == loaded_args, f"Model config does not match the one saved at {config_path}."

    # Print run information
    print(f"Starting from {args.starting_video}, generating {args.length} frames, and storing the results at {args.out}.")
    ...
    for cnt, frame_idx in enumerate(range(0, args.length, args.file_length)):
        ...
        assert not path.exists(), f"About to generate video #{video_index_offset + cnt} at {path} but this file already exists."
