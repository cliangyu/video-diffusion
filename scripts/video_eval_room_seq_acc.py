...
def _smooth_seq(seqs):
    # seqs shape: (..., N)
    kernel = [i / 5.0 for i in range(1, 6)] + [i / 5.0 for i in reversed(range(1, 5))]
    ss = np.zeros(
        list(seqs.shape[:-1]) + [seqs.shape[-1] + len(kernel) - (len(kernel) % 2)]
    )
...
def print_metrics(name, metrics):
    ...
    print("{} : acc={}/{} = {}%".format(name, count, len(met), count / len(met) * 100))
...
if __name__ == "__main__":
    ...
    seqs_gt = np.stack([data_fetch[i]["gt"].transpose([0, 2, 3, 1]) for i in range(len(data_fetch))])
    ...
    for idx in range(args.num_samples):
        seqs_pred = np.stack([get_sample(data_fetch[i]["preds"], idx=idx).transpose([0, 2, 3, 1]) for i in range(len(data_fetch))])
        seqs_pred = (seqs_pred * 255).astype(np.uint8)
        hallways_pred, room_stay_pred, hallway_stay_pred, recovery_pred = verify_hallway(
            seqs_pred, hallway_entry_thresh, hallway_out_thresh
        )
        acc_list.append(
            get_single_stats({
    ...
    print(f"{np.mean(acc_list) * 100}% +- {np.std(acc_list) / np.sqrt(args.num_samples - 1) * 100}")
