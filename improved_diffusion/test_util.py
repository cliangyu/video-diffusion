...
class Protect(FileLock):
    """Given a file path, this class will create a lock file and prevent race
    conditions using a FileLock.

    The FileLock path is automatically inferred from the file path.
    """
...
def get_model_results_path(args, postfix=''):
    """Given arguments passed to an evaluation run, returns the path to the
    results path. The path has the format
    "results/<checkpoint_dir_subpath>/checkpoint name" where.
    ...
    if args.eval_dir is None:
        ...
        if name.endswith('latest'):
            checkpoint_step = torch.load(args.checkpoint_path,
...
def get_eval_run_identifier(args, postfix=''):
    ...
    if hasattr(args,
               'dataset_partition') and args.dataset_partition == 'train':
        res = 'trainset_' + res
...
def tensor2pil(tensor, drange=[0, 1]):
    """Given a tensor of shape (Bx)3xwxh with pixel values in drange, returns a
    PIL image of the tensor.

    Returns a list of images if the input tensor is a batch.
    Args:
    ...
    img_batch = ((img_batch - drange[0]) / (drange[1] - drange[0]) * 255
                 )  # img_batch with pixel values in [0, 255]
...
def tensor2mp4(tensor, path, drange=[0, 1], random_str=''):
    ...
    os.system(
        f'ffmpeg -y -hide_banner -loglevel error -i {gif_path} -r 10 -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" {path}'
    )
