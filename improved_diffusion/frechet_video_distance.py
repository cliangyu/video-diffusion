...
def frechet_statistics_to_frechet_metric(stat_1, stat_2):
    ...
    if not np.isfinite(covmean).all():
        print(f'WARNING: fid calculation produces singular product; '
...
def mmd2(K_XX, K_XY, K_YY, unit_diagonal=False, mmd_est='unbiased'):
    assert mmd_est in ('biased', 'unbiased',
                       'u-statistic'), 'Invalid value of mmd_est'
...
def polynomial_mmd(features_1, features_2, degree, gamma, coef0):
    k_11 = polynomial_kernel(features_1,
                             features_1,
                             degree=degree,
                             gamma=gamma,
                             coef0=coef0)
    k_22 = polynomial_kernel(features_2,
                             features_2,
                             degree=degree,
                             gamma=gamma,
                             coef0=coef0)
    k_12 = polynomial_kernel(features_1,
                             features_2,
                             degree=degree,
                             gamma=gamma,
                             coef0=coef0)
