...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        self.valid_microbatch = (args.valid_microbatch
                                 if args.valid_microbatch > 0 else batch_size)
        self.lr = lr
        ...
        self.resume_checkpoint = (resume_checkpoint if resume_checkpoint else
                                  find_resume_checkpoint(self._args))
        ...
        with RNG(0):
            self.valid_batches = [
                next(self.data)[0][:self.valid_microbatch]
    ...
    def _load_ema_parameters(self, rate):
        ...
        if ema_checkpoint:
            ...
            print(
                f'Failed to find EMA checkpoint for rate {rate} and main checkpoint {main_checkpoint}.'
            )
            raise Exception
        if dist.is_initialized():
    ...
    def _load_optimizer_state(self):
        main_checkpoint = self.resume_checkpoint
        filename = ('opt_latest.pt' if self._args.save_latest_only else
                    f'opt_{(self.step):06d}.pt')
        opt_checkpoint = bf.join(bf.dirname(main_checkpoint), filename)
    ...
    def sample_some_indices(self, max_indices, T):
        ...
        max_scale = T / (s - 0.999)
        if (self.mask_distribution in [
                'one-group',
                'differently-spaced-groups',
                'differently-spaced-groups-no-marg',
        ] or 'linspace' in self.mask_distribution):
    ...
    def sample_all_masks(
        ...
        p_observed_latent_marg = th.tensor(
            [0.33, 0.33, 0.33] if self.do_inefficient_marg else [0.5, 0.5, 0])
        N = self.max_frames
        B, T, *_ = batch1.shape
        masks = {
            k: th.zeros_like(batch1[:, :, :1, :1, :1])
            for k in ['obs', 'latent', 'kinda_marg']
        }
        for obs_row, latent_row, marg_row in zip(
                *[masks[k]
                  for k in ['obs', 'latent', 'kinda_marg']]):  # for each video
            if 'autoregressive' in self.mask_distribution:
                n_obs = int(self.mask_distribution.split('-')[1])
                n_latent = self.max_frames - n_obs
                start_i = th.randint(low=0,
                                     high=T - self.max_frames + 1,
                                     size=())
                obs_row[start_i:start_i + n_obs] = 1.0
                latent_row[start_i + n_obs:start_i + n_obs + n_latent] = 1.0
            elif 'linspace-no-obs' in self.mask_distribution:
                low, high, n = map(
                    int,
                    self.mask_distribution.split('-')
                    [-3:])  # for frameskip set T=frameskip*(max_frames-1)+1
                indices = th.linspace(low, high, n).long()
                latent_row[indices] = 1.0
            elif 'linspace' in self.mask_distribution:
                low, high, n = map(
                    int,
                    self.mask_distribution.split('-')
                    [1:])  # for frameskip set T=frameskip*(max_frames-1)+1
                indices = th.linspace(low, high, n).long()
                latent_row[indices] = 1.0
                while th.rand(size=()) > 0.5 and N - sum(obs_row) > 1:
                    index_indices = th.tensor(
                        self.sample_some_indices(max_indices=N -
                                                 sum(obs_row).int().item() - 1,
                                                 T=N)).long()
                    obs_row[indices[index_indices]] = 1.0
                    latent_row[indices[index_indices]] = 0.0
                ...
                indices = np.random.choice(T,
                                           size=n_frames.item(),
                                           replace=False)
                obs_row[indices[:n_obs]] = 1.0
                latent_row[indices[n_obs:]] = 1.0
                ...
                obs_row[obs_indices] = 1.0
                latent_indices = np.setdiff1d(indices, obs_indices)
                latent_row[latent_indices] = 1.0
            elif 'groups' in self.mask_distribution:
                latent_row[self.sample_some_indices(max_indices=N, T=T)] = 1.0
                while True:
                    mask_i = th.distributions.Categorical(
                        probs=p_observed_latent_marg).sample()
                    mask = [obs_row, latent_row, marg_row][mask_i]
                    indices = th.tensor(
                        self.sample_some_indices(max_indices=N, T=T))
                    taken = (obs_row[indices] + latent_row[indices] +
                             marg_row[indices]).view(-1)
                    indices = indices[
                        taken ==
                        0]  # remove indices that are already used in a mask
                    if len(indices) > N - sum(obs_row) - sum(latent_row) - sum(
                            marg_row):
                        break
                    mask[indices] = 1.0
            else:
        ...
        represented_mask = (masks['obs'] + masks['latent'] +
                            masks['kinda_marg']).clip(max=1)
        if not gather:
            return batch1, masks['obs'], masks['latent'], masks['kinda_marg']
        (
            represented_mask,
            batch,
            (obs_mask, latent_mask, kinda_marg_mask),
            frame_indices,
        ) = self.gather_unmasked_elements(
            represented_mask,
            batch1,
            batch2,
            (masks['obs'], masks['latent'], masks['kinda_marg']),
        )
    ...
    def gather_unmasked_elements(self, mask, batch1, batch2, tensors):
        ...
        effective_T = (self.max_frames if self.pad_with_random_frames else
                       mask.sum(dim=1).max().int())
        ...
        for b in range(B):
            ...
            indices[b, instance_T:] = (th.randint_like(indices[b, instance_T:],
                                                       high=T)
                                       if self.pad_with_random_frames else 0)
            new_batch[b, :instance_T] = batch1[b][mask[b] == 1]
            new_batch[b, instance_T:] = (batch1 if batch2 is None else
                                         batch2)[b][indices[b, instance_T:]]
            for new_t, t in zip(new_tensors, tensors):
                new_t[b, :instance_T] = t[b][mask[b] == 1]
                new_t[b, instance_T:] = t[b][indices[b, instance_T:]]
        return new_mask.view(B, effective_T, 1, 1,
                             1), new_batch, new_tensors, indices
    ...
    def forward_backward(self):
        ...
        for i in range(0, batch1.shape[0], self.microbatch):
            ...
            micro2 = batch2[i:i +
                            self.microbatch] if batch2 is not None else None
            (
                micro,
                frame_indices,
                obs_mask,
                latent_mask,
                kinda_marg_mask,
            ) = self.sample_all_masks(micro1, micro2)
            ...
            loss_mask = ((1 - obs_mask - kinda_marg_mask)
                         if self.pad_with_random_frames else latent_mask)
    ...
    def _master_params_to_state_dict(self, master_params):
        if self.use_fp16:
            master_params = unflatten_master_params(self.model.parameters(),
                                                    master_params)
    ...
    def _state_dict_to_master_params(self, state_dict):
        params = [
            state_dict[name] for name, _ in self.model.named_parameters()
        ]
        if self.use_fp16:
    ...
    def make_interesting_masks(self, batch):
        ...
        for i in range(n_masks):
            ...
            obs_mask[i, :n_obs * spacing:spacing] = 1.0
            latent_mask[i, n_obs * spacing:self.max_frames *
                        spacing:spacing] = 1.0
        return {
            'obs': obs_mask,
            'latent': latent_mask,
            'kinda_marg': kinda_marg_mask
        }
    ...
    def log_samples(self):
        ...
        self.model.load_state_dict(
            copy.deepcopy(self._master_params_to_state_dict(
                self.ema_params[0])))
        ...
        batch, orig_batch, frame_indices, obs_mask, latent_mask, kinda_marg_mask = map(
            repeat,
            [
                batch, orig_batch, frame_indices, obs_mask, latent_mask,
                kinda_marg_mask
            ],
        )
        ...
        attns = {
            k: th.cat([a[k] for a in attns], dim=0)
            for k in attns[0].keys()
        }
        ...
        for b in range(len(batch)):
            ...
            vis_all[b, existing_frame_indices] = vis[
                b, :len(existing_frame_indices)]
        ...
        n_samples_with_preset_masks = len(
            set_masks['obs']) * self.n_valid_repeats
        if n_samples_with_preset_masks > 0:
            gather_and_log_videos('sample/',
                                  vis[:n_samples_with_preset_masks],
                                  log_as='video')
        ...
        for k, v in spatial_attn.items():
            logger.logkv(
                k,
                wandb.Image(
                    concat_images_with_padding(v.unsqueeze(1),
                                               horizontal=False).cpu()),
            )
        for k, attn in frame_attn.items():
            ...
            for fi, attn_matrix, ax in zip(frame_indices.cpu().numpy(),
                                           attn.cpu(), axes):
                n_frames = attn_matrix.shape[-1]
                ax.imshow(attn_matrix, vmin=0, cmap='binary_r')
                for axis, set_ticks, set_labels, set_lim in [
                    ('x', ax.set_xticks, ax.set_xticklabels, ax.set_xlim),
    ...
    def visualise(self):
        ...
        batch, obs_mask, latent_mask, kinda_marg_mask = self.sample_all_masks(
            batch, gather=False)
        vis = th.ones_like(batch)
        vis[obs_mask.expand_as(batch) == 1] = batch[obs_mask.expand_as(batch)
                                                    == 1]
        for quartile in [0, 1, 2, 3, 3.99]:
            t = th.tensor(self.diffusion.num_timesteps * (quartile / 4)).int()
            xt = self.diffusion.q_sample(batch, t=t)
            vis[latent_mask.expand_as(batch) == 1] = xt[latent_mask.expand_as(
                batch) == 1]
            gather_and_log_videos(
                f'visualise/inputs-q{quartile}',
                vis,
                log_as='array',
                pad_dim_h=4,
                pad_dim_v=4,
        ...
        red = th.tensor([1.0, 0.0, 0.0]).view(1, 1, 3, 1, 1)
        blue = th.tensor([0.0, 1.0, 0.0]).view(1, 1, 3, 1, 1)
        vis = th.ones_like(vis)
        vis_red = vis * red
        vis_blue = vis * blue
        vis[obs_mask.expand_as(batch) == 1] = vis_red[obs_mask.expand_as(batch)
                                                      == 1]
        vis[latent_mask.expand_as(batch) == 1] = vis_blue[
            latent_mask.expand_as(batch) == 1]
        gather_and_log_videos(
            'visualise/mask',
            vis,
            log_as='array',
        ...
        for i in range(n_masks):
            with RNG(i):
                batch, obs_mask, latent_mask, kinda_marg_mask = self.sample_all_masks(
                    batch, gather=False)
            obs_indices += [[
                list(layer.nonzero(as_tuple=True)[0].flatten().numpy())
            ] for layer in obs_mask.flatten(start_dim=2)]
            lat_indices += [[
                list(layer.nonzero(as_tuple=True)[0].flatten().numpy())
            ] for layer in latent_mask.flatten(start_dim=2)]
...
def _mark_as_observed(images, color=[1.0, -1.0, -1.0]):
...
def concat_images_with_padding(images,
                               horizontal=True,
                               pad_dim=1,
                               pad_val=0,
                               pad_ends=False):
...
def gather_and_log_videos(name,
                          array,
                          log_as='both',
                          pad_dim_h=1,
                          pad_dim_v=1,
    ...
    if dist.is_initialized():
        dist.all_gather(gathered_arrays,
                        array)  # gather not supported with NCCL
    ...
    if log_as in ['video', 'both']:
        ...
        final_frame[..., ::2,
                    1::2] = 255  # checkerboard pattern to mark the end
        for i, video in enumerate(videos):
            logger.logkv(name + f'video-{i}',
                         wandb.Video(th.cat([video, final_frame], dim=0)))
...
def get_blob_logdir(args):
    ...
    assert os.path.exists(
        root_dir
    ), "Must create directory 'checkpoints' or specify existing DIFFUSION_BLOB_LOGDIR"
...
def find_resume_checkpoint(args):
    """If there are checkpoints saved in get_blob_logdir(), will return the
    latest one."""
...
def find_ema_checkpoint(main_checkpoint, step, rate, save_latest_only):
    ...
    filename = (f'ema_{rate}_latest.pt'
                if save_latest_only else f'ema_{rate}_{(step):06d}.pt')
